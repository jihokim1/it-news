...
export default async function RankingPage({ searchParams }: Props) {
  ...
  let whereCondition: any = {
    ...
    category: { contains: currentCategoryLabel === "game" ? "게임" 
  ...
  const getRankStyle = (rank: number) => {
    if (rank === 1) return "text-yellow-500 text-3xl"; // 금
    if (rank === 2) return "text-slate-400 text-2xl";   // 은
    if (rank === 3) return "text-amber-600 text-2xl";   // 동
    return "text-slate-800 text-lg";                    // 일반
  };
        ...
        <div className="container mx-auto max-w-5xl px-4 py-4">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <h1 className="text-2xl font-black tracking-tight text-slate-900">차트</h1>
